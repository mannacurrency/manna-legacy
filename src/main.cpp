...
int nCoinbaseMaturity = COINBASE_MATURITY_MANNA;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // manna: moved from original location in peercoin code
...
bool CBlock::AcceptBlock()
{
    ...
    // manna: check for proof-of-stake blocks before allowed
    if (nHeight < START_BLOCK_PROOF_OF_STAKE)
    {
        if (IsProofOfStake())
            return DoS(100, error("AcceptBlock() : Proof-of-stake before allowed"));
    }
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Manna", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Manna", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // peercoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
...
string GetWarnings(string strFor)
{
    ...
    // peercoin: if detected unmet upgrade requirement enter safe mode
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // manna: allow only proof-of-work blocks until proof-of-stake starting block
    if (fProofOfStake && nBestHeight < START_BLOCK_PROOF_OF_STAKE)
        return NULL;
